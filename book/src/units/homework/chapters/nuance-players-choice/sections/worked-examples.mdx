...
I chose to make an admittedly weird game/art thing where you control a flower
launcher to shoot seeds into the air, which eventually land and plant pretty
flowers.

This features interactivity, image and sound files, dynamic graphics and sound,
realistic particle physics, and no particular goal. I'm showing this as an
example of a non-trivial p5 sketch that you can study and perhaps get ideas
from.

Use the arrow keys to control the launcher's angle and power, and the Space key
to launch a seed.

At the top right you'll see music controls, which you can use to make it
spicier. The music was literally the first hit of a Google search for 'random
mp3'.

The image and sound assets are loaded in a `setup` function marked `async` to
stake the claim that it contains asynchronous code; the specific calls that do
the long-running loading code use `await`. This makes the `setup` function wait
until each of the awaited functions is finished.

There are two sound files: one for the music, and another 'landing' sound that
plays when a seed hits the ground. The file that I found off of
[freesound.org](https://freesound.org/) had between 200-300 milliseconds of
silence before the sound actually happens; I addressed this by using the sound
object's `jump()` function to scoot in a little bit. This is a hack - I _should_
have used an audio editor to snip off the dead space at the beginning.

The dynamic sound is created using oscillators. That's a pretty deep topic, so
I'm going to leave that one quite simple. When the projectile goes up/down, the
pitch gets higher/lower; it also trails off in volume as it gets farther away
from the flower cannon.

Enjoy.

<Ask id="blossom-boom" mode="untracked" title="Blossom Boom">

```p5exercise size="large" sound
// this sketch features the world's first flower launcher,
// spreading the seeds of blooms within range of its
// blossom boomer.

const flowerFiles = [
  "/sketch-assets/blossom-boom/flower-1.png",
  "/sketch-assets/blossom-boom/flower-2.png",
  "/sketch-assets/blossom-boom/flower-3.png",
];

const landFile = "/sketch-assets/blossom-boom/land.wav";

const themeMusicFile = "/sketch-assets/blossom-boom/random.mp3";
