...
function makeBoolProp(name: string) {
  return {
    type: "Property",
    key: { type: "Identifier", name },
...
function makeExerciseAttr(id: string, initialCode: string, title?: string, size?: string, autorun?: boolean) {
  const properties = [
...
export function remarkP5Exercise() {
  return (tree: Root) => {
    visit(tree, "code", (node: Code, index: number | undefined, parent: Parent | undefined) => {
      ...
      const { id, title, size, autorun } = parseMeta(node.meta)
      if (!id) {
      ...
      const attributes: any[] = [makeExerciseAttr(id, node.value, title, size, autorun)]
