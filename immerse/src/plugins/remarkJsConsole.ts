...
function makeExerciseAttr(id: string, initialCode: string, title?: string, size?: string, autorun?: boolean, hoverInfo?: boolean, solutionTo?: string) {
  ...
  return {
    ...
    value: {
      ...
      value: `{ type: "console", id: ${JSON.stringify(id)} }`,
      data: {
        estree: {
          type: "Program",
          body: [
...
export function remarkJsConsole() {
  return (tree: Root) => {
    visit(tree, "code", (node: Code, index: number | undefined, parent: Parent | undefined) => {
      if (node.lang !== "jsconsole" || !parent || index == null) return

      const { id, title, size, autorun, hoverInfo, solutionTo } = parseMeta(node.meta)
      if (!id) {
        console.warn("[remarkJsConsole] missing required id= attribute, skipping fence")
        return
      }
