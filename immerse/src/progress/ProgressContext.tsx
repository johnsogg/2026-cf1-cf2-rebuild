...
export function getSectionStatus(urlPath: string): AttemptState {
  ...
  if (states.some((s) => s === "attempted" || s === "complete"))
    return "attempted"
  return "idle"
}
...
export const ProgressProvider = ({ children }: { children: ReactNode }) => {
  ...
  const registerExerciseInSection = useCallback(
    (exerciseId: string, urlPath: string) => {
      const ids = getStorageValue((d) => d.sections?.[urlPath]?.exercises ?? [])
  ...
  return (
    <ProgressContext.Provider
      value={{ version, registerExerciseInSection, notifyExerciseChange }}
    >
      {children}
    </ProgressContext.Provider>
  )
